...
const headCells = [
  ...
  { id: 'salaryStatus', numeric: true, disablePadding: false, label: 'Salary Payment?' },
];
...
export default function EnhancedTable() {
  ...
  const cancelPayment = (event) => {
   let myArray = payments.filter(ar => !selected.find(rm => (rm === ar) ))
    setPayments(myArray);
  };
  const makePayment = (event) => {
    setPayments(Object.assign([], selected));
   };
  ...
  return (
    <div className={classes.root}>
                ...
                .map((row, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell align="left">{payments.some(item => item === row.id)?'true':'false'}</TableCell>
                    </TableRow>
                  );
                })}
              {emptyRows > 0 && (
      ...
      <Button onClick={makePayment} variant="primary">Make Payment</Button>
  <Button onClick={cancelPayment} className="btn" variant="danger">Cancel Payment</Button>
    </div>
  );
}